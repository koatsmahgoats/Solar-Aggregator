...
#        nVars[i] = variables[i]
    ...
    ##############Altered Parameters###########################
    # Tilt of Subarray 
    ssc.data_set_number(dat,'subarray1_tilt', variables[3])
    # Nameplate Capacity
   # ssc.data_set_number(dat, 'system_capacity', 5.0)
    #system_size = system_size + 0.1
    ssc.data_set_number(dat, 'modules_per_string', variables[2])
    #Strings in parallel    
    ssc.data_set_number(dat, 'strings_in_parallel',variables[1])
    # Number of Inverter
    ssc.data_set_number(dat, 'inverter_count', inverter_num)
    # Azimuth of subarray 
    #0=N,90=E,180=S,270=W
    ssc.data_set_number(dat,'subarray1_azimuth', variables[4])
    
    panel_num = mods_string * string_num
...
#############################Write Data to csv
    ...
    results_borg = [npv,adjusted_installed_cost,payback,annual_avg_solar_fraction]
...
## Change to Raw Input
max_inverter_v= 600
mppt_inverter_low = 250
mod_temp_coeff = -0.177
mod_nom_voc = 49.2
mod_nom_vmp = 39.8

mod_lower_bound = int(round(mppt_inverter_low/ (mod_nom_vmp - ((max_temp-25) * mod_temp_coeff))))
