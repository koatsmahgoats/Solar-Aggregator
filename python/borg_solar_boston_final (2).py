...
def solar_opt(*variables):
    ...
    if system_capacity >5:
        msrp = 2800
    else:
        msrp = (system_capacity*400) + (system_capacity*200) # Massachusetts Solar Power Rebate at $400/kW up to 5kW
    if msrp > 4250:
        msrp = 4250
    rebate_amount = msrp + (1/3)*(msrp) # Accounts for upfront rebate per Watt in $
    ...
    # Flat Buy Rate $
    ssc.data_set_number(dat,"ur_flat_buy_rate", 0.152)
    # Flat Sell Rate $
    ...
    # ITC State Max Value
    ssc.data_set_number(dat,"itc_sta_percent_maxvalue",1000)
    # System Capacity
    ssc.data_set_number(dat,"system_capacity", system_capacity)
    #Loan Term
