...
#C:\temp\ssc-sdk-2014-1-21\win64
    ...
    def data_set_string(self, p_data, name, value):
        self.pdll.ssc_data_set_string( c_void_p(p_data), c_char_p(name.encode()), c_char_p(value.encode()) )
        #self.pdll.ssc_data_set_string( c_void_p(p_data), c_char_p(name), c_char_p(value) )

    def data_set_number(self, p_data, name, value):
        self.pdll.ssc_data_set_number( c_void_p(p_data), c_char_p(name.encode()), c_number(value) )
...
#            arr[i] = c_number(parr[i])
        ...
        return self.pdll.ssc_data_set_array( c_void_p(p_data), c_char_p(name.encode()),pointer(arr), c_int(count))
    ...
    def data_set_matrix(self,p_data,name,mat):
        ...
        for r in range(nrows):
            for c in range(ncols):
                ...
                idx=idx+1
        return self.pdll.ssc_data_set_matrix( c_void_p(p_data), c_char_p(name.encode()),pointer(arr), c_int(nrows), c_int(ncols))


    def data_get_string(self, p_data, name):
        self.pdll.ssc_data_get_string.restype = c_char_p
        #return self.pdll.ssc_data_get_string( c_void_p(p_data), c_char_p(name.encode()) )
        t = self.pdll.ssc_data_get_string( c_void_p(p_data), c_char_p(name.encode()) )
        return t.decode('utf-8')

    def data_get_number(self, p_data, name):
        val = c_number(0)
        self.pdll.ssc_data_get_number( c_void_p(p_data), c_char_p(name.encode()), byref(val) )
        return val.value

    def data_get_array(self,p_data,name):
        count = c_int()
        self.pdll.ssc_data_get_array.restype = POINTER(c_number)
        parr = self.pdll.ssc_data_get_array( c_void_p(p_data), c_char_p(name.encode()), byref(count))
#        arr = []
...
#            arr.append( float(parr[i]) )
    ...
    def data_get_matrix(self,p_data,name):
        ...
        parr = self.pdll.ssc_data_get_matrix( c_void_p(p_data), c_char_p(name.encode()), byref(nrows), byref(ncols) )
        idx = 0
        mat = []
    ...
    def entry_name(self,p_entry):
        ...
        t = self.pdll.ssc_entry_name( c_void_p(p_entry) )
        return t.decode('utf-8')
    ...
    def entry_description(self,p_entry):
        ...
        t = self.pdll.ssc_entry_description( c_void_p(p_entry) )
        return t.decode('utf-8')
    ...
    def module_create(self,name):
        ...
        return self.pdll.ssc_module_create( c_char_p(name.encode()) )
    ...
    def module_log( self, p_mod, index ):
        ...
        t = self.pdll.ssc_module_log( c_void_p(p_mod), c_int(index), byref(type), byref(time) )
        if t is not None:
            return t.decode('utf-8')
        else:
            return t
...
if __name__ == "__main__":
    ...
    def print_data(d):
        ...
        while (name != None):
            type = ssc.data_query(d,name.encode())
            outstr = '\t'
    ...
    def simtest():
        ...
        while (1):
            ...
            while (1):
                ...
                if ( ssc.info_var_type(inf) == PySSC.INPUT ):
                    print('\tInput: \'' + ssc.info_name(inf) + '\' type(' + str(ssc.info_data_type(inf)) + ')  '  + '  (' + ssc.info_units(inf) + ')')                    
                    print('\tInput: \'' + ssc.info_name(inf) + '\' type(' + str(ssc.info_data_type(inf)) + ')  ' + ssc.info_label(inf) + '  (' + ssc.info_units(inf) + ')')
                    
                else:
                    print('\tOutput: \'' + ssc.info_name(inf) + '\' type(' + str(ssc.info_data_type(inf)) + ')  ' + '  (' + ssc.info_units(inf) + ')')
        ...
        mod = ssc.module_create("utilityrate3")
        ...
        ssc.data_set_array(dat, "degradation", [0.5])
        ...
        if ssc.module_exec(mod, dat) == 0:
            ...
            print('UtilityRate Simulation ok, year1_energy_value=  ($) ', ssc.data_get_array(dat, "annual_energy_value")[0])
        ...
        ssc.data_set_number(dat, "annual_energy_value", 25)
        ssc.data_set_number(dat, "system_use_lifetime_output", 25)
...
# Test program 'main'
    ...
    while (inf != None):
        if (ssc.info_var_type(inf) == PySSC.INPUT):
            print('\tInput: \'' + ssc.info_name(inf) + '\' type(' + ssc.info_data_type(inf) + ')')
