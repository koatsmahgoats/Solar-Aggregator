...
def solar_opt(*variables):
    ...
    panel_num = variables[1] * variables[0] # Number of Panels
    system_capacity = panel_num*315/1000 # System Capacity in kWh
    rebate_amount = (system_capacity*1000)*0.295 # Accounts for upfront rebate per Watt in $
    install_cost = panel_num *315 * cost_per_watt - (rebate_amount) # Install cost in $
    
    inverter_num = int(variables[0]) # Number of inverters 
    dat = ssc.data_create() # API Data Creation
    #######Weather Data#############################
    # Pulls from TMY data storage
    ssc.data_set_string(dat, 'solar_resource_file', '../../languages/python/23169.tm2')
    ssc.data_set_string(dat, 'file_name', '../../languages/python/23169.tm2')
    ...
    with open('USA_NV_Las.Vegas-McCarran.Intl.AP.723860_TMY3_BASE.csv') as csvfile:
    ...
    # Rate Escalation %
    ssc.data_set_array(dat,"rate_escalation", [3.5])
    #Enable Net Metering
    ...
    ssc.data_set_number(dat,"ur_excess_monthly_energy_or_dollars",1)
    # Net Metering Year-end Sell Rate
    ssc.data_set_number(dat,"ur_nm_yearend_sell_rate", nm_sell_rate)
    # Monthly Fixed Charge $
    ssc.data_set_number(dat,"ur_monthly_fixed_charge", 17.90)
    # Flat Buy Rate $
    ssc.data_set_number(dat,"ur_flat_buy_rate", 0.10566)
    # Flat Sell Rate $
    ssc.data_set_number(dat,"ur_flat_sell_rate", nm_sell_rate)
    ...
    ssc.data_set_number(dat,"analysis_period", analysis_period)
    # Annual Energy Value
    ssc.data_set_array(dat,"annual_energy_value", annual_energy_value)
    # Federal Tax Rate
    ssc.data_set_number(dat,"federal_tax_rate", 30)
    # State Tax Rate
    ssc.data_set_number(dat,"state_tax_rate", 0)
    # Real Discount Rate
    ...
    # ITC State Percent
    ssc.data_set_number(dat,"itc_sta_percent", 0)
    # ITC State Max Value
    ssc.data_set_number(dat,"itc_sta_percent_maxvalue",0)
    # System Capacity
    ssc.data_set_number(dat,"system_capacity", system_capacity)
    # Sales Tax Rate
    ssc.data_set_number(dat,"sales_tax_rate", 6.85)
    #Loan Term
    ssc.data_set_number(dat,"loan_term",15)
    #Loan Interest Rate %
    ssc.data_set_number(dat,"loan_rate",3)
    # Market Type
    #0=residential,1=comm.
    ssc.data_set_number(dat,"market", 0)
    #Property Tax Rate
    ssc.data_set_number(dat,"property_tax_rate", 3.0562)
    # PBI Utility provided amt in $/kWh
    ssc.data_set_array(dat,"pbi_uti_amount", [0.0159])
    # PBI Term in $/kWh
    ssc.data_set_number(dat,"pbi_uti_term", analysis_period)
    # PBI Taxable at Federal Level
    # 0 = no, 1 =yes 
    ssc.data_set_number(dat,"pbi_uti_tax_fed", 0)
    # PBI Taxable at State Level
    # 0 = no, 1 =yes 
    ssc.data_set_number(dat,"pbi_uti_tax_sta", 0)
    #Property Tax % of System Cost Assesed
    ssc.data_set_number(dat,"prop_tax_cost_assessed_percent", 35)
    # Mortgage
    # 0=standard loan,1=mortgage
    ssc.data_set_number(dat,"ur_monthly_fixed_charge", 0)
    # Total Installed Cost
    ssc.data_set_number(dat,"total_installed_cost", install_cost)
    # Gen
    ssc.data_set_array(dat,"gen", gen)
    # Degradation
    ssc.data_set_array(dat,"degradation", degradation)
...
####### Iterate and return results###############
    ...
    results_borg = [-1*npv,adjusted_installed_cost,-1*annual_avg_solar_fraction] # Results for BORG
 
    ssc.module_free(cashloan) # Free cashloan module and memory
    ssc.data_free(dat) # Free Data
...
epsilon1 = 7  # for total cost (obj 1)
epsilon2 = 7  # Installed Cost
epsilon3 = 7 # % of Satisfied Load
